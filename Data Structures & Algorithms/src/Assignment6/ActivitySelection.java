...
public class ActivitySelection {
	...
	class Activity implements Comparable<Activity>  {
		...
		public boolean equals(Object o) {
			...
			return a.start == this.start && a.finish == this.finish && a.weight 
					== this.weight;
		}
		@Override     
		public int compareTo(Activity activity) {          
			return (this.getFinish() < activity.getFinish() ? -1 : 
				(this.getFinish() == activity.getFinish() ? 0 : 1));     
		}       
		@Override     
		public String toString() {         
			return " Start: " + this.getStart() + ", Finish: " + this.getFinish() +
	...
	public void addActivity(int start, int finish, int weight) {
		activities.add(new Activity(start, finish, weight));
	}
	
	public Activity returnActivity(int start, int finish, int weight) {
		return new Activity(start, finish, weight);
	}
	/**
	 * Returns the maximal possible list of activities that do not overlap with 
each other.
	 * Implements the GREEDY activity selection algorithm as we saw it in class;
	 * either an iterative or recursive implementation is fine.
	 * The goal is to select a maximum list of activities that 
	...
	public int findlastfinishingjob(int inds, int indf, int nextstart) {
		int max_fin = -1;
		int max_ind = -1;
		for (int i = inds; i <= indf; i++) {
			int fin = activities.get(i).getFinish();
	...
	/**
	 * Function selectActivitiesDynamic_maxSumWeights
	 * Returns the highest sum of weights for a list of activities
	 * that do not overlap. Note: This uses the weight for each Activity.
	 * Implements the DYNAMIC activity selection algorithm.
	 * This finds the activities that maximize the sum of weights
	 * and also do not overlap with each other!
	 * @return The highest sum of weights for any list of Activities that do not 
overlap
	 */
	public int selectActivitiesDynamic_maxSumWeights() {
		//First sort in ascending order by the finish time
		//(How does this sorting work? See the compareTo method in the Activityclass).
		Collections.sort(activities);
		int n = activities.size();
		int optimalWeightsum[] = new int[n+1];
		optimalWeightsum[0] = 0;
		int lastfinishingjob[] = new int[n];
		...
		for(int i = 1; i< n; i++){
			...
			lastfinishingjob[i] = findlastfinishingjob(0, i-1, 
					activities.get(i).getStart());
		}
		//Compute the optimal weights
		for(int j = 1; j<=n; j++){
			int weightsumExcluding = optimalWeightsum[j-1];
			int weightsumIncluding = activities.get(j-1).getWeight();
			if(lastfinishingjob[j-1] != -1){
				weightsumIncluding += optimalWeightsum[lastfinishingjob[j-
				                                                        1]+1];
			}
			optimalWeightsum[j] = Math.max(weightsumIncluding, 
					weightsumExcluding);
		}
	...
	/**
	 * Function selectActivitiesDynamic_listMaxSumWeights
	 * Returns the list of activities that have the highest sum of weights
	 * and do not overlap. Note: This uses the weight for each Activity.
	 * Implements the DYNAMIC activity selection algorithm.
	 * THIS IS THE SAME AS selectActivitiesDynamic_maxSumWeights above, 
	 * except this returns the actual list of activities chosen.
	 * The goal is to find the activities that maximize the sum of weights
	 * and also do not overlap with each other!
	 * @return The list of the Activities that were chosen with the highest sum 
of weights.
	 */
	public List<Activity> selectActivitiesDynamic_listMaxSumWeights() {
		...
		//(How does this sorting work? See the compareTo method in the Activity class).
		Collections.sort(activities);
		List<Activity> maxWeightList = new ArrayList<Activity>();
		int n = activities.size();
		...
		for(int j = 1; j<=n; j++){
			...
			/**
			 * TODO At this point you may need to keep track of some information, so 
			 * in the end you can append the included Activities to the maxWeightList.
			 */
			
			if((Math.max(weightsumIncluding, weightsumExcluding) == weightsumIncluding) && (lastfinishingjob[j-1] != -1)) {
				maxWeightList.removeAll(activities);
				
				maxWeightList.add(activities.get(lastfinishingjob[j-1]));
				maxWeightList.add(activities.get(j-1));
				} 
			}
		
		Collections.sort(maxWeightList);
		return maxWeightList;
	}
	public static void main(String[] args) {
		ActivitySelection select = new ActivitySelection();
		/**
		 * Add a series of activities with their start, finish times and 
weights.
		 */
		...
		/**
		 * Run the greedy activity selection, as we saw it in class.
		 * Check that the maximal number of activities that do not overlap is 
returned.
		 * The assert statements show you what result to expect from the 
		 * greedy activity selection algorithm.
		 */
		List<Activity> list = select.selectActivitiesGreedy();
		...
		/**
		 * Run the dynamic activity selection, as we saw it in class.
		 * Check if the maximal sum of weights for a list of activities
		 * that do not overlap is returned.
		 * The assert statements show you what result to expect from the 
		 * dynamic activity selection algorithm.
		 */
		int maxweight = select.selectActivitiesDynamic_maxSumWeights();
		//Assert the result is the sum of weights for selection of activities (1,48,1000) and (49,99,1000)
		assertEquals(maxweight, 2000);
		/**
		 * Check that the correct activity list that maximizes the sum of 
weights is returned.
		 */
		List<Activity> list2 = 
				select.selectActivitiesDynamic_listMaxSumWeights();
